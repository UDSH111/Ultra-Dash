...
- [Development guidelines](#development-guidelines)
    - [General Ultradash Coin](#general-ultradash-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Ultradash Coin uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Ultradash Coin is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/ultradash-project/ultradash/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_ultradash
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_ultradash --log_level=test_suite
$ valgrind -v --leak-check=full src/ultradashd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_ultradash.coverage/index.html`.
```
...
- BitcoinMiner : Generates UDSHs (if wallet is enabled).
...
However, in open source software such as Ultradash Coin, where everyone uses
...
pay attention to for reviewers of Ultradash Coin code.

General Ultradash Coin
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Ultradash Coin uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Ultradash Coin so that they can be integrated
...
file descriptors it will cause problems with Ultradash's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Ultradash-Project/Ultradash.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `ultradash-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `ultradash-cli` and the GUI debug console use this table to determine how to

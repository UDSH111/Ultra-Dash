...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zudshObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zudshObj.push_back(Pair(to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zudshObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zUDSHsupply", zudshObj));
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "  \"zUDSHsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zUDSH denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zUDSH denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zUDSH denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zUDSH denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zUDSH denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zUDSH denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zUDSH denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zUDSH denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zUDSH denominations\n"
            "  }\n"
            "}\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZUDSH_FAILED_ACCUMULATOR_INITIALIZATION);

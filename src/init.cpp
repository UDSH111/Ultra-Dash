...
void Shutdown()
{
	...
	if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
		lightWorker.StopLightZudshThread();
	}
#ifdef ENABLE_WALLET
	delete pwalletMain;
	pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
	...
	strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the UDSH and zUDSH money supply statistics") + " " + _("on startup"));
	...
	string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, ultradash, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, precompute, staking)"; // Don't translate these and qt below
	if (mode == HMM_BITCOIN_QT)
		debugCategories += ", qt";
	...
	strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Ultradash specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
	...
	strUsage += HelpMessageOpt("-zudshstake=<n>", strprintf(_("Enable or disable staking functionality for zUDSH inputs (0-1, default: %u)"), 1));
	...
	strUsage += HelpMessageOpt("-backupzudsh=<n>", strprintf(_("Enable automatic wallet backups triggered after each zUDSH minting (0-1, default: %u)"), 1));
	strUsage += HelpMessageOpt("-precompute=<n>", strprintf(_("Enable precomputation of zUDSH spends and stakes (0-1, default %u)"), 1));
	...
	strUsage += HelpMessageOpt("-zudshbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zUDSH backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
#define PROCESS_DEP_ENABLE 0x00000001
	...
	while (!fLoaded) {
		...
		do {
			try {
				...
				//Ultradash specific: zerocoin and spork DB's
				...
				if (GetBoolArg("-reindexmoneysupply", false)) {
					RecalculateZUDSHMinted();
					RecalculateZUDSHSpent();
	...
	if (fDisableWallet) {
		...
		zwalletMain = new CzUDSHWallet(pwalletMain->strWalletFile);
		...
		//Inititalize zUDSHWallet
		uiInterface.InitMessage(_("Syncing zUDSH wallet..."));

		pwalletMain->InitAutoConvertAddresses();

		bool fEnableZUdshBackups = GetBoolArg("-backupzudsh", true);
		pwalletMain->setZUdshAutoBackups(fEnableZUdshBackups);

		//Load zerocoin mint hashes to memory
		pwalletMain->zudshTracker->Init();
	...
	*/
	...
	if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
		...
		lightWorker.StartLightZudshThread(threadGroup);
	}

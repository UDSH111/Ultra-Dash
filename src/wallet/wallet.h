...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zudshspends need to use the cs_spendcache
...
// Possible states for zUDSH send
enum ZerocoinSpendStatus {
    ZUDSH_SPEND_OKAY = 0,                            // No error
    ZUDSH_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZUDSH_WALLET_LOCKED = 2,                         // Wallet was locked
    ZUDSH_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZUDSH_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZUDSH_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZUDSH_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZUDSH_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZUDSH_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZUDSH_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZUDSH_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZUDSH_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZUDSH_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZUDSH_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZUDSH_SPENT_USED_ZUDSH = 14,                      // Coin has already been spend
    ZUDSH_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZUDSH_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZUdshBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZUDSHOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzudshAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzUDSHTracker> zudshTracker;
    ...
    void setZWallet(CzUDSHWallet* zwallet)
    {
        zwalletMain = zwallet;
        zudshTracker = std::unique_ptr<CzUDSHTracker>(new CzUDSHTracker(strWalletFile));
    }

    CzUDSHWallet* getZWallet() { return zwalletMain; }
    ...
    void setZUdshAutoBackups(bool fEnabled)
    {
    ...
    /** zUDSH reset */
    boost::signals2::signal<void()> NotifyzUDSHReset;

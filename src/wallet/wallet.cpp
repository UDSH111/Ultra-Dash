...
#include "zudshchain.h"

#include "denomination_functions.h"
#include "libzerocoin/Denominations.h"
#include "zudsh/zudshwallet.h"
#include "zudsh/zudshtracker.h"
...
{
    return zudshTracker->HasSerial(bnSerial);
}
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    ...
    bool fCheckZUDSH = GetBoolArg("-zapwallettxes", false);
    if (fCheckZUDSH)
        zudshTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zudsh
            if (fCheckZUDSH) {
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount, bool fPrecompute)
{
    ...
    if (GetBoolArg("-udshstake", true) && !fPrecompute) {
    ...
    /* Disable zUDSH Staking
    //zUDSH
    if ((GetBoolArg("-zudshstake", true) || fPrecompute) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        //Only update zUDSH set once per update interval
        bool fUpdate = false;
        ...
        set<CMintMeta> setMints = zudshTracker->ListMints(true, true, fUpdate);
        ...
        )
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (Stake(stakeInput.get(), nBits, block.GetBlockTime(), nTxNewTime, hashProofOfStake)) {
            ...
            //Masternode payment
            FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZUDSH());

            {
                TRY_LOCK(zudshTracker->cs_spendcache, fLocked);
                if (!fLocked)
                    continue;
            ...
            //Mark mints as spent
            if (stakeInput->IsZUDSH()) {
                CZUdshStake* z = (CZUdshStake*)stakeInput.get();
...
bool CWallet::CreateZUDSHOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZUDSH(denomination, coin, dMint);
...
bool CWallet::CheckCoinSpend(libzerocoin::CoinSpend& spend, libzerocoin::Accumulator& accumulator, CZerocoinSpendReceipt& receipt)
{
    if (!spend.Verify(accumulator)) {
        receipt.SetStatus(_("The transaction did not verify"), ZUDSH_BAD_SERIALIZATION);
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST && IsSerialKnown(spend.getCoinSerialNumber())) {
        //Tried to spend an already spent zUDSH
        receipt.SetStatus(_("The coin spend has been used"), ZUDSH_SPENT_USED_ZUDSH);
        uint256 hashSerial = GetSerialHash(spend.getCoinSerialNumber());
        if(!zudshTracker->HasSerialHash(hashSerial))
            return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());

        CMintMeta meta = zudshTracker->Get(hashSerial);
        meta.isUsed = true;
        if (!zudshTracker->UpdateState(meta))
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZUDSH_TXMINT_GENERAL);
    ...
    while (nLockAttempts < 100) {
        TRY_LOCK(zudshTracker->cs_spendcache, lockSpendcache);
        if (!lockSpendcache) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            CMintMeta meta = zudshTracker->Get(GetSerialHash(mint.GetSerialNumber()));
            CoinWitnessData *coinWitness = zudshTracker->GetSpendCache(meta.hashStake);
            ...
            if (!GenerateAccumulatorWitness(coinWitness, mapAccumulators, pindexCheckpoint)) {
                receipt.SetStatus(_("Couldn't generate the accumulator witness"),
                                  ZUDSH_FAILED_ACCUMULATOR_INITIALIZATION);
           ...
           /* if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {*/
                ...
                if (!mint.GetKeyPair(key))
                    return error("%s: failed to set zUDSH privkey mint version=%d", __func__, nVersion);
            ...
            try {
                ...
                if (!CheckCoinSpend(spend, accumulator, receipt)) {
                    receipt.SetStatus(_("CoinSpend: failed check"), ZUDSH_SPEND_ERROR);
                    return error("%s : %s", __func__, receipt.GetStatusMessage());
                }
                ...
                receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), ZUDSH_INVALID_WITNESS);
                return error("%s : %s", __func__, receipt.GetStatusMessage());
            }
        }
        break;
    ...
    if (nLockAttempts == 100) {
        ...
        receipt.SetStatus(_("could not get lock on cs_spendcache"), ZUDSH_TXMINT_GENERAL);
        return false;
    }
    ...
    receipt.SetStatus(_("Spend Valid"), ZUDSH_SPEND_OKAY); // Everything okay
    ...
    while (nLockAttempts < 100) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
                receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZUDSH_TXMINT_GENERAL);
                return false;
            } else if (mapBlockIndex.count(hashBlock) < 1) {
                // check that this mint made it into the blockchain
                receipt.SetStatus(_("Mint did not make it into blockchain"), ZUDSH_TXMINT_GENERAL);
                return false;
            }
            ...
            if (outputIndex == -1) {
                receipt.SetStatus(_("Pubcoin not found in mint tx"), ZUDSH_TXMINT_GENERAL);
                return false;
            }

            mint.SetOutputIndex(outputIndex);
            CTxIn in;
            if(!ZUDSHModule::createInput(in, mint, hashTxOut)){
                receipt.SetStatus(_("Cannot create public spend input"), ZUDSH_TXMINT_GENERAL);
                return false;
            }
            vin.emplace_back(in);
...
string CWallet::GetUniqueWalletBackupName(bool fzudshAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzudshAuto ? "-autozudshbackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}

void CWallet::ZUdshBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozudshbackup-%d.dat", i);
        ...
        if (filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozudshbackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozudshbackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
    ...
    if(!GetArg("-zudshbackuppath", "").empty()) {
        filesystem::path customPath(GetArg("-zudshbackuppath", ""));
...
{
    CMintMeta meta;
    if (!zudshTracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (zudshTracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zudshTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zudshTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zudshTracker->UpdateState(meta);
    } else {
...
bool CWallet::SetMintUnspent(const CBigNum& bnSerial)
{
    ...
    CMintMeta meta = zudshTracker->Get(hashSerial);
    zudshTracker->SetPubcoinNotUsed(meta.hashPubcoin);
    return true;
}

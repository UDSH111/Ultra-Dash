...
public:
    ...
    bool WriteZUDSHSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZUDSHSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZUDSHSeed_deprecated(uint256& seed);
    bool EraseZUDSHSeed();
    bool EraseZUDSHSeed_deprecated();

    bool WriteZUDSHCount(const uint32_t& nCount);
    bool ReadZUDSHCount(uint32_t& nCount);

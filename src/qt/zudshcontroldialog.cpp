...
#include "ui_zudshcontroldialog.h"
...
std::set<std::string> ZUdshControlDialog::setSelectedMints;
std::set<CMintMeta> ZUdshControlDialog::setMints;

bool CZUdshControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZUdshControlDialog::COLUMN_DENOMINATION || column == ZUdshControlDialog::COLUMN_VERSION || column == ZUdshControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZUdshControlDialog::ZUdshControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZUdshControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZUdshControlDialog::~ZUdshControlDialog()
{
    delete ui;
}

void ZUdshControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZUdshControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZUdshControlWidgetItem* itemDenom(new CZUdshControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZUdshControlWidgetItem *itemMint = new CZUdshControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zudshTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zudshTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zUDSH.";
            else if (!mint.isSeedCorrect)
                strReason = "The zUDSH seed used to mint this zUDSH is not the same as currently hold in the wallet";
            else
...
void ZUdshControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZUdshControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZUdsh_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZUdshControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZUdshControlDialog::GetSelectedMints()
{
...
void ZUdshControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

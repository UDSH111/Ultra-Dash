...
#include <QtWidgets>
#include <zudsh/deterministicmint.h>
                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zUDSH ought to be enough for anybody." - Bill Gates, 2017
    ui->zUDSHpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzUDSHSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zUDSHpayAmount->setFocus();
    }
}
/* disable MINT
 *
void PrivacyDialog::on_pushButtonMintzUDSH_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zUDSH is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zUDSH, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zUDSH...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zUDSH in ") +
...
void PrivacyDialog::on_pushButtonSpendzUDSH_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zUDSH is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zUDSH, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zUDSH
        sendzUDSH();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zUDSH
    sendzUDSH();
}

void PrivacyDialog::on_pushButtonZUdshControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZUdshControlDialog* zUdshControl = new ZUdshControlDialog(this);
    zUdshControl->setModel(walletModel);
    zUdshControl->exec();
}

void PrivacyDialog::setZUdshControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzUdshSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzUDSH()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Ultradash Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zUDSHpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zUDSH is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " UDSH </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zUDSH</b>";
    ...
    // use mints from zUDSH selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZUdshControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZUdshControlDialog::GetSelectedMints();
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zUDSH transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zUDSH was spent successfully update the addressbook with the label
    ...
    // Clear zudsh selector in case it was used
    ZUdshControlDialog::setSelectedMints.clear();
    ui->labelzUdshSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zUDSH Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Udsh, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.IsZerocoinMint())
            strStats += tr("zUDSH Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zudshTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zUDSH </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zUDSH "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zUDSH "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zUDSH "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zUDSH </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zUDSH </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zUDSH </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzUDSHSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzUDSH->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zUDSH
        //ui->pushButtonMintzUDSH->setEnabled(false);
        //ui->pushButtonMintzUDSH->setToolTip(tr("zUDSH is currently disabled due to maintenance."));

        // Spend zUDSH
        ui->pushButtonSpendzUDSH->setEnabled(false);
        ui->pushButtonSpendzUDSH->setToolTip(tr("zUDSH is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zUDSH
        //ui->pushButtonMintzUDSH->setEnabled(true);
        //ui->pushButtonMintzUDSH->setToolTip(tr("PrivacyDialog", "Enter an amount of UDSH to convert to zUDSH", 0));

        // Spend zUDSH
        ui->pushButtonSpendzUDSH->setEnabled(true);
        ui->pushButtonSpendzUDSH->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}

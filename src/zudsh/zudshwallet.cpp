...
CzUDSHWallet::CzUDSHWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zudsh seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZUDSHSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZUDSHSeed_deprecated()) {
                    LogPrintf("%s: Updated zUDSH seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zudsh seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zudsh wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzUDSHWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZUDSHCount(nCountLastUsed);
    else if (!walletdb.ReadZUDSHCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzUDSHWallet::Lock()
{
    seedMaster = 0;
}

void CzUDSHWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzUDSHWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZUDSH(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzUDSHWallet::LoadMintPoolFromDB()
{
...
void CzUDSHWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzUDSHWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzUDSHWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zudshTracker->HasPubcoinHash(pMint.first)) {
...
bool CzUDSHWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZUDSH(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zudshTracker which also adds to database
    pwalletMain->zudshTracker->Add(dMint, true);
...
void CzUDSHWallet::SeedToZUDSH(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzUDSHWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzUDSHWallet::GenerateDeterministicZUDSH(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzUDSHWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzUDSHWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzUDSHWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());

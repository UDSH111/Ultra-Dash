...
CzUDSHTracker::CzUDSHTracker(std::string strWalletFile)
{
...
CzUDSHTracker::~CzUDSHTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzUDSHTracker::Init()
{
...
bool CzUDSHTracker::Archive(CMintMeta& meta)
{
...
bool CzUDSHTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzUDSHTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzUDSHTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzUDSHTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzUDSHTracker::GetSpendCache(const uint256& hashStake)
{
...
std::vector<uint256> CzUDSHTracker::GetSerialHashes()
{
...
CAmount CzUDSHTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzUDSHTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzUDSHTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzUDSHTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzUDSHTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzUDSHTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzUDSHTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzUDSHTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzUDSHTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzUDSHTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzUDSHTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzUDSHWallet* zUDSHWallet)
{
    bool iszUDSHWalletInitialized = (NULL != zUDSHWallet);
    CMintMeta meta;
    ...
    if (! iszUDSHWalletInitialized)
        zUDSHWallet = new CzUDSHWallet(strWalletFile);
    meta.isSeedCorrect = zUDSHWallet->CheckSeed(dMint);
    if (! iszUDSHWalletInitialized)
        delete zUDSHWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzUDSHTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzUDSHTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzUDSHTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzUDSHTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzUDSHTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzUDSHTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzUDSHWallet* zUDSHWallet = new CzUDSHWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zUDSHWallet);
        }
        delete zUDSHWallet;
        LogPrint("zero", "%s: added %d dzudsh from DB\n", __func__, listDeterministicDB.size());
    }
